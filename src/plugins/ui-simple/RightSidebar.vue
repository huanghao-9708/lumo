    ...
    <div class="flex-1 min-h-0 relative z-10">
      ...
      <template v-if="playerStore.activeRightTab === '歌词'">
        <div class="flex flex-col h-full">
          ...
          <div class="mb-6 flex flex-col shrink-0">
            <!-- Album Art Square -->
            <div class="w-full aspect-square bg-[#eae8e1] mb-5 overflow-hidden relative border border-[#dcdad1]/50 shadow-sm transition-colors duration-700" :class="playerStore.currentTrack?.coverColor">
              <img 
            ...
            <h2 class="font-serif italic text-3xl text-black mb-1.5 truncate">{{ playerStore.currentTrack?.title || 'No Track' }}</h2>
            ...
            <div class="w-8 h-px bg-[#dcdad1] mt-4"></div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar -mr-4 pr-4">
            <div class="space-y-6 text-[13px] leading-relaxed pb-8">
              <p 
                ...
                :class="[
                  idx === playerStore.activeLyricIndex ? 'font-serif italic font-bold text-[18px] text-black tracking-wide' : 'text-[#777]',
                  line.text === '' ? 'h-4' : ''
                ]"
              >
                {{ line.text }}
      ...
      <template v-else-if="playerStore.activeRightTab === '播放队列'">
        <div class="flex flex-col h-full">
          ...
          <div class="mb-6 shrink-0">
            <p class="text-[9px] font-bold tracking-[0.2em] text-[#a0a0a0] mb-1.5 uppercase">PLAYBACK QUEUE</p>
            <h2 class="font-serif italic text-3xl text-black leading-none">播放队列</h2>
            <p class="text-[10px] text-[#888] mt-2 tracking-wider">共 {{ playerStore.queue.length }} 首曲目</p>
            <div class="w-8 h-px bg-[#dcdad1] mt-4"></div>
          </div>
              ...
              >
                <div class="flex-1 min-w-0 pr-4">
                  <p 
                    ...
                    :class="playerStore.currentTrack?.id === track.id && playerStore.currentIndex === index ? 'font-serif italic font-bold text-black text-[15px]' : 'text-[#777] group-hover:text-black'"
                  >
                    {{ track.title }}
                  </p>
                ...
                <div v-if="playerStore.currentTrack?.id === track.id && playerStore.currentIndex === index" class="shrink-0 flex items-center justify-center pl-2">
                  <AudioLines v-if="playerStore.isPlaying" class="w-4 h-4 text-black animate-pulse stroke-[1.5]" />
      ...
      <template v-else-if="playerStore.activeRightTab === '文件信息'">
        <div class="flex flex-col h-full">
          <!-- Info Header -->
          <div class="mb-6 shrink-0">
            <p class="text-[9px] font-bold tracking-[0.2em] text-[#a0a0a0] mb-1.5 uppercase">FILE METADATA</p>
            <h2 class="font-serif italic text-3xl text-black leading-none">文件信息</h2>
            <div class="w-8 h-px bg-[#dcdad1] mt-4"></div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar -mr-4 pr-4">
            ...
            <div v-if="playerStore.currentTrack" class="flex items-center gap-4 mb-6 p-3 bg-black/[0.02] border border-[#f0eee6] rounded-md">
              <div class="w-16 h-16 bg-[#eae8e1] overflow-hidden relative shrink-0 border border-[#dcdad1]/50 shadow-sm" :class="playerStore.currentTrack.coverColor">
                <img 
                  v-if="playerStore.currentTrack.cover_artwork_id"
                  :src="getArtworkUrl(playerStore.currentTrack.cover_artwork_id)"
                  class="absolute inset-0 w-full h-full object-cover"
              ...
              <div class="min-w-0">
                <h3 class="font-serif italic text-lg text-black truncate leading-tight mb-1">{{ playerStore.currentTrack.title }}</h3>
                <p class="text-[10px] font-semibold text-[#555] truncate uppercase tracking-wider">{{ playerStore.currentTrack.artist }}</p>
                <p class="text-[9px] text-[#888] truncate mt-0.5">{{ playerStore.currentTrack.album }}</p>
              </div>
            </div>
            ...
            <div class="space-y-5 text-[11px] tracking-wider uppercase text-[#555] pb-8">
              ...
              <div>
                ...
                <span class="text-black">{{ playerStore.currentTrack?.duration || '-' }}</span>
              </div>
              <div>
                <span class="text-[#a0a0a0] block mb-1 text-[9px] font-bold">曲目编号</span>
                <span class="text-black">ID: #{{ playerStore.currentTrack?.id || '-' }}</span>
              </div>
            </div>
          </div>
        </div>
