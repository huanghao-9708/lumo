...
impl TrackRepo {
    pub fn get_tracks_paginated(
        ...
        if let Some(pattern) = keyword_pattern {
            ...
            let mut stmt = conn.prepare(&sql)?;
            let tracks =
                stmt.query_map(params![limit, offset], crate::repositories::map_track_row)?;
            for t in tracks {
                result.push(t?);
            }
        }
    ...
    pub fn record_play(
        ...
        track_id: i64,
        duration_ms: i64,
        requested_media_file_id: Option<i64>,
    ) -> rusqlite::Result<()> {
        // 更新总体计数字段
        conn.execute(
    ...
    pub fn save_play_queue(conn: &Connection, track_ids: &[i64]) -> rusqlite::Result<()> {
        ...
        {
            let mut stmt =
                tx.prepare("INSERT INTO play_queue (track_id, position) VALUES (?1, ?2)")?;
    ...
    /// 智能歌单：最近添加（tracks.added_at 降序，字段有索引）。
    ...
    /// 智能歌单：最近播放（按 tracks.last_played_at，曲库全集口径）。
    ...
    /// 智能歌单：未曾播放（play_count = 0，按添加时间正序展示）。
    pub fn get_never_played_tracks(
        conn: &Connection,
        limit: i64,
    ) -> rusqlite::Result<Vec<TrackDTO>> {
        let mut stmt = conn.prepare("
    ...
    fn ranked_details(
        ...
        ids: &[i64],
        order_by: &str,
    ) -> rusqlite::Result<Vec<RankedTrackDTO>> {
        if ids.is_empty() {
            return Ok(Vec::new());
        }
    ...
    pub fn get_recent_added_ranked(
        conn: &Connection,
        limit: i64,
    ) -> rusqlite::Result<Vec<RankedTrackDTO>> {
        let ids: Vec<i64> = {
    ...
    pub fn get_last_played(conn: &Connection) -> rusqlite::Result<Option<RankedTrackDTO>> {
        ...
        match id {
            Some(id) => Ok(
                Self::ranked_details(conn, &[id], "t.last_played_at DESC, t.id DESC")?
                    .into_iter()
                    .next(),
            ),
            None => Ok(None),
