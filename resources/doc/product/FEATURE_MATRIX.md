...
| G-19 | 本地 stable 与 CI 的 `rust-toolchain@stable` 漂移 5 个版本（本地 clippy 0.1.93 / CI 1.98），`-D warnings` 门禁只在 CI 暴露：`6533502` 带进 `for_kv_map`（`scanner.rs:456`）与 `useless_borrows_in_formatting`（`track_repo.rs:383`）两处，本地 fmt/clippy/test 全绿、CI 三平台全红 | P2（首发 v1.8.2-beta.1 的 preflight 因此被打回，publish 作业全部跳过） | **已修** `9fa25fa`：两处告警清零，改发 `v1.8.2-beta.2`。残余：CI 的 stable 未钉版本，漂移会继续攒出"本地永远验不出"的红——I2 二选一，用 `rust-toolchain.toml` 钉版本并让本地跟随，或在 CI 显式指定版本并写进发布门禁说明 |
| G-20 | 门禁脚本自身依赖新语法：`check-network-registry.mjs` 对生成器直接 `.map()`（Node ≥22 才有 Iterator helpers），而 `release.yml` 的 preflight 固定 `setup-node: 20` → 该步抛 `TypeError: walk.map is not a function`，v1.8.2-beta.2 发布被打回；同一脚本在 `ci.yml` 的治理作业里全绿，因为那个作业**没写** `setup-node`、吃的是 runner 默认 Node 24——同一门禁在两条流水线上结论相反 | P1（发布链路可用性：连续两次 tag 被门禁自身拦下） | **已修** `9ecf2c8`：先 `[...walk(...)]` 展开成数组再映射，Node 20/24 一致；本地用删除 `Iterator.prototype.*` 的 preload 复现原崩溃并回归通过。残余：两条流水线的 Node 口径仍不一致（显式 20 / 隐式默认），`package.json` 也没有 `engines` 下限——I2 收口，二选一：两条流水线写同一 `node-version` + 声明 `engines`，或统一用 runner 默认 |
| G-21 | §2 的 Beta tag 形态与 Windows 打包器冲突：MSI（WiX）要求预发布标识符是**纯数字且 ≤65535**，`1.8.2-beta.N` 在 `tauri build` 阶段就被打回 → `v1.8.2-beta.3` 的 windows 作业零产物（macOS/Linux 正常），Beta 用户拿不到主力平台的包 | P1（发布链路可用性：唯一可发的桌面平台缺包） | **已修** `d8f4c82`：预发布 tag 下 Windows 只出 NSIS（本机用 `1.8.2-beta.3` 实跑 `makensis` 验证可产出 `Lumo_1.8.2-beta.3_x64-setup.exe`），稳定 tag 仍按 bundle targets 全出。残余已闭环：Stable 首发 `v1.8.2` 实跑通过，Release 产物同时含 `Lumo_1.8.2_x64_en-US.msi` 与 NSIS setup |
| G-22 | 发布链路的 Android 作业断链：`publish-android` 卡在 `android-actions/setup-android@v3`——它仍要装上游已不再提供的 `tools` 包，`sdkmanager` 报 `Warning: Failed to find package 'tools'` 后 exit 1（`v1.8.1` 之前能出 APK，故这是 runner 镜像 / SDK 仓库侧变化打断了流水线，不是本仓库代码）。连带：Android 作业失败让整条 Publish Release 记为 failure，但桌面三平台产物已正常上传 | P2（Beta 通道缺 Android 包；与 §2「Beta 不得暴露未验证平台」不冲突，但 Stable 前必须能出） | **已修** `d23b469`：`setup-android` 升 v4（默认仅装 platform-tools，显式请求 tools 也只告警跳过）。`v1.8.2` tag 实跑全绿，产出签名包 `Lumo-v1.8.2-arm64-app-universal-release.apk` + SHA256SUMS。残余：§2 仍需明确 Android 在真机矩阵（G-01）前只走 Nightly；APK 真机冒烟见 V-01 |
